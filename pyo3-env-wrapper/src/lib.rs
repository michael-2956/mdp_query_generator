...
use equivalence_testing::rl_env::environment::QueryCreationEnvironment;
use pyo3::prelude::*;
...
#[pyclass]
pub struct QueryCreationEnvironmentWrapper {
    env: QueryCreationEnvironment,
}

#[pymethods]
impl QueryCreationEnvironmentWrapper {
    /// Constructor visible to Python.  e.g. `env = my_rust_env.QueryCreationEnvironmentWrapper(0)`
    #[new]
    pub fn new(config_path: &str) -> Self {
        // let inner_constraint = query_constraint.into_inner();
        QueryCreationEnvironmentWrapper {
            env: QueryCreationEnvironment::new(
                // inner_constraint,
...
fn pyo3_env_wrapper(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<QueryCreationEnvironmentWrapper>()?;
